module priority_encoder 
    #(parameter IN_WIDTH= `RS_SIZE)
    //synopsys template
    (req,enc);

    input [IN_WIDTH-1:0] req;
    output [$clog2(IN_WIDTH)-1:0] enc; 
    logic [IN_WIDTH-1:0] one_hot_code;

    wand_sel #(.WIDTH(IN_WIDTH)) ps (.req(req),.gnt(one_hot_code));
    pe #(.IN_WIDTH(IN_WIDTH)) encoder(.gnt(one_hot_code), .enc(enc));
